...
const Register = () => {
    ...
    switch (name) {
      case "name":
        ...
        if (!patterns.name.test(value))
          return "Name should be 3-30 characters long and contain only letters";
        return "";
      ...
      case "password":
        ...
        if (!patterns.password.hasUpperCase.test(value))
          return "Password must contain at least one uppercase letter";
        if (!patterns.password.hasLowerCase.test(value))
          return "Password must contain at least one lowercase letter";
        if (!patterns.password.hasNumber.test(value))
          return "Password must contain at least one number";
        if (!patterns.password.hasSpecialChar.test(value))
  ...
  const checkPasswordStrength = (password) => {
    const checks = [
      ...
      {
        regex: patterns.password.hasSpecialChar,
        label: "One special character",
      },
      {
        regex: new RegExp(`.{${patterns.password.minLength},}`),
        label: "8+ characters",
      },
    ];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success("Registration successful!", {
        description: "Welcome to Shatranj CRM",
      });
      navigate("/");
    } catch (err) {
      const errorMessage =
      ...
      <div className="hidden md:flex md:w-1/2 bg-gradient-to-tl from-blue-800 to-blue-600 p-12 items-center justify-center">
        <div className="max-w-md text-white space-y-6">
          <h1 className="text-4xl font-bold">
            Start Your Journey with Shatranj CRM
          </h1>
